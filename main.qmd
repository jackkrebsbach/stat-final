...
data_full$Y <- factor(data_full$Y, levels = c("1", "0"))
```
...
```{r}
numeric_features <- data_full %>%
  dplyr::select(-Y) %>%
  slice_sample(n = 100) 
#pairs(numeric_featuress)

gaussian_features <- numeric_features %>%
...
train.X <- model.matrix(Y ~ ., data_full)[,-1]
train.Y <- as.numeric(data_full$Y)
...
```{r}
tsk <- as_task_classif(data_full, target = "Y")
...
learner <- mlr3::lrn("classif.qda", predict_type = "prob")

optimizer <- opt("grid_search", resolution = 50)
graph <- po("learner_cv", learner) %>>% 
         po("tunethreshold", 
            measure = msr("classif.fbeta", beta = 1, na_value = 0),
            optimizer = optimizer)
...
learner <- mlr3::lrn("classif.ranger", num.trees = 300, mtry = 3,
                     min.node.size = 2, max.depth = 10, predict_type = "prob")
pset <- ps(
  num.trees = p_int(200, 350),
  mtry = p_int(3, 5),  
  min.node.size = p_int(2, 8),
  max.depth = p_int(3, 8)
)
